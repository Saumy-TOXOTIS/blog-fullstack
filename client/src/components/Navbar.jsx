        ...
        <div className="flex justify-between items-center h-20">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="hidden md:flex items-center space-x-2">
              <DesktopNavLink to="/about" text="About" color="teal" onClick={() => navigate('/about')} icon={coloredIcon(<svg className="h-4 w-4 md:h-5 md:w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>, 'teal')} />
              {isAuthenticated && (
                <>
                  {navLinks.map(link => <DesktopNavLink key={link.to} {...link} onClick={() => navigate(link.to)} icon={coloredIcon(link.icon, link.color)} />)}
